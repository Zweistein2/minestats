...
class MinecraftStatsCategoryModel(val stats: JsonObject, val category: CategoryKeys) {
    fun forStat(key: Key): Long {
        when (key) {
            ...
            is ItemKeys -> { require(category in listOf(CategoryKeys.BROKEN, CategoryKeys.PICKED_UP, CategoryKeys.DROPPED, CategoryKeys.CRAFTED, CategoryKeys.USED))
                { "Item stat keys can only be called in the broken, picked_up, dropped, crafted and used stat categories" } }
            is CustomKeys -> { require(category in listOf(CategoryKeys.CUSTOM))
                { "Custom stat keys can only be called in the custom stat category" } }
            is MobKeys -> { require(category in listOf(CategoryKeys.KILLED, CategoryKeys.KILLED_BY))
