...
class ServerController(
    ...
    fun getServerStats(model: Model): String {
        val runtimeInMilliseconds = measureTimeMillis {
            monitoring.withTimer("showServerStats", "frontend") {
                val multicraftAPI =
                    MulticraftAPI(multicraftProperties.apiUrl, multicraftProperties.user, multicraftProperties.key)
                ...
                val currentMemoryUsage =
                    ((maxMemoryUsage.times(serverResources?.data?.memory ?: 1.0f)) / 100.0f).roundToInt()
                val latestServerHistoryStats =
                    if (serverStatsHistory.isNotEmpty()) serverStatsHistory.last() else ServerStatsModel(
                        LocalDateTime.now(), 0, 0, 0, 0.0f, 0, Properties()
                    )
                val worldSize =
                    latestServerHistoryStats.worldsize.toDouble().div(1024.0).div(1024.0).div(1024.0).toFloat()
                val tps = latestServerHistoryStats.tps
                val maxPlayers = (latestServerHistoryStats.properties["max-players"] as String).toInt()

                val serverStatsMeta = ServerStatsMetaModel(
                    serverStatus?.data?.onlinePlayers ?: 0,
                    worldSize,
                    currentMemoryUsage,
                    maxMemoryUsage,
                    tps,
                    serverResources?.data?.cpu ?: 0.0f
                )

                val serverStatusInfo = MinecraftQuery(minestatProperties.queryIp, minestatProperties.queryPort).status()

                val playerlist =
                    serverStatusInfo?.playerInfo?.players?.toList()?.stream()?.map { it.name }?.toList() ?: listOf()
                ...
                val serverStatsHistoryForDays = serverStatsHistory.stream()
                    .filter { it.timestamp.isAfter(LocalDateTime.now().minusDays(minestatProperties.historyDays)) }
                    .toList()
                ...
                val cpuChartPayload = serverService.getChartPayload("Prozessorauslastung",
                    "CPU",
                    "%",
                    0,
                    200 * minestatProperties.coreCount,
                    serverStatsHistoryForDays.map { it.timestamp.toString() }.toList(),
                    serverStatsHistoryForDays.map { it.cpu }.toList()
                )
                val ramChartPayload =
                    serverService.getChartPayload("Speicherauslastung", "RAM", "MB", 0, maxMemoryUsage,
                        serverStatsHistoryForDays.map { it.timestamp.toString() }.toList(),
                        serverStatsHistoryForDays.map { it.ram }.toList()
                    )
                val worldSizeChartPayload =
                    serverService.getChartPayload("Weltengröße", "GB", "GB", 0, ceil(worldSize).toInt(),
                        serverStatsHistory.stream().map { it.timestamp.toString() }.toList(),
                        serverStatsHistory.stream()
                            .map { it.worldsize.toDouble().div(1024.0).div(1024.0).div(1024.0).toFloat() }.toList()
                    )
